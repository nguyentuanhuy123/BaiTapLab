...
import { listCart, clearCart } from "@/src/db/cart.repo";
...
export default function CartScreen() {
  ...
  const handleCheckout = async () => {
    ...
    setLoading(true);
    try {
      const orderId = await checkoutFromCart("Thanh toán đơn hàng");
      Alert.alert("✅ Thành công", `Đã tạo đơn hàng #${orderId}`);
      await loadCart();
    } catch (err: any) {
  ...
  return (
    <View style={{ flex: 1, padding: 20 }}>
      <Text style={{ fontSize: 20, fontWeight: "bold", marginBottom: 10 }}>🛒 Giỏ hàng</Text>
